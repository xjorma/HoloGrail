...
#else
extern extern int    voxie_laser_read (int id, voxie_laser_t *las);
#endif
...
extern void (__cdecl *voxie_drawmeshtex)(voxie_frame_t *vf, char *fnam, poltex_t *vt, int vtn, int *mesh, int meshn, int flags, int col);
...
struct kzfile_t;
extern void *(__cdecl *kzsetfil       )(FILE *);
...
{
#if defined(_WIN32)
	hvoxie = LoadLibrary(L"voxiebox.dll"); if (!hvoxie) return(-1);
